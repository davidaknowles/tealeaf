...
for fold in 0 1; do
  ...
  smoothing_map=""
  if [[ -n "${SMOOTHING_MAP_NAME}" ]]; then
    smoothing_map="${fold_root}/${SMOOTHING_MAP_NAME}"
  fi
  uncertainty_scale_map=""
  ...
  merge_command="module load ${PYTHON_MODULE} && source ${HOME}/venv/cpa_blackwell/bin/activate && PYTHONPATH=${REPO_ROOT} python ${REPO_ROOT}/extra_scripts/merge_paired_path_test.py --shards ${output_root}/shard_* --output-dir ${fold_root}/${OUTPUT_TAG} ${moderation_flag} --calibration empirical"
