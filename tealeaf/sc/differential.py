...
def gaussian_mixed_model_lrt(
    ...
    *,
    ...
    """Test estimated multivariate responses with a Gaussian mixed model.

    ``values`` is an ``n`` by ``d`` matrix, ``covariances`` contains ``n``
    known ``d`` by ``d`` measurement covariance matrices, and ``design`` is
    ``n`` by ``p``. The marginal model has an isotropic ``d``-dimensional
    random intercept for each cluster and isotropic extra residual variance.
    Variance components are maximum-likelihood estimates fitted separately
    under the nested null and alternative, rather than REML estimates shared
    by a subsequent Wald test.

    The score Bayes factors integrate the tested coefficient vector under
    zero-centered Gaussian priors. ``effect_prior_covariance`` is a positive
    definite ``q`` by ``q`` covariance shape for the ``q`` tested coefficients;
    each effect scale multiplies its standard deviations and is evaluated at
    the fitted null.
    """
    values = np.asarray(values, dtype=float)
    covariances = np.asarray(covariances, dtype=float)
    ...
    tested_columns = np.asarray(tested_columns, dtype=np.int64)
    clusters = np.asarray(clusters)
    prior_scales = np.asarray(effect_prior_scales, dtype=float)
    if random_intercept_solver not in {"dense", "woodbury"}:
        raise ValueError("random_intercept_solver must be dense or woodbury")
    if values.ndim != 2:
        raise ValueError("values must be samples by log-ratio dimensions")
    n_samples, dimension = values.shape
    if covariances.shape != (n_samples, dimension, dimension):
    ...
    if design.ndim != 2 or design.shape[0] != n_samples:
        raise ValueError("design and values must align")
    if len(clusters) != n_samples:
        raise ValueError("clusters and values must align")
    if not len(tested_columns):
        raise ValueError("at least one tested column is required")
    if np.any(tested_columns < 0) or np.any(tested_columns >= design.shape[1]):
        raise ValueError("tested_columns are outside the design")
    if np.linalg.matrix_rank(design) != design.shape[1]:
        raise ValueError("the alternative design is rank deficient")
    if prior_scales.ndim != 1 or not len(prior_scales):
        raise ValueError("effect_prior_scales must be a nonempty vector")
    if not np.isfinite(prior_scales).all() or np.any(prior_scales <= 0):
        raise ValueError("effect prior scales must be positive and finite")
    null_design = np.delete(design, tested_columns, axis=1)
    tested_design = design[:, tested_columns]
    degrees_of_freedom = tested_design.shape[1] * dimension
    if not null_design.shape[1]:
        raise ValueError("at least one untested design column is required")
    if np.linalg.matrix_rank(null_design) != null_design.shape[1]:
        raise ValueError("the null design is rank deficient")
    if effect_prior_covariance is None:
        prior_covariance_shape = np.eye(degrees_of_freedom)
    else:
        prior_covariance_shape = np.asarray(
            effect_prior_covariance, dtype=float
        )
        if prior_covariance_shape.shape != (
            degrees_of_freedom, degrees_of_freedom
        ):
            raise ValueError("effect_prior_covariance has the wrong shape")
        prior_covariance_shape = 0.5 * (
            prior_covariance_shape + prior_covariance_shape.T
        )
        try:
            scipy.linalg.cholesky(prior_covariance_shape, check_finite=True)
        except np.linalg.LinAlgError as error:
            raise ValueError(
                "effect_prior_covariance must be positive definite"
            ) from error
    cluster_rows = [
        np.flatnonzero(clusters == cluster) for cluster in np.unique(clusters)
    ]

    def cluster_solve(rows, cluster_variance, residual_variance, right_hand_side):
        """Apply one random-intercept block inverse with Woodbury."""
        ...
        if random_intercept_solver == "dense":
            size = len(rows)
            variance = scipy.linalg.block_diag(*[
                covariances[row] + residual_variance * np.eye(dimension)
                for row in rows
            ...
            try:
                factor = scipy.linalg.cho_factor(
                    variance, lower=True, check_finite=False
                )
            except np.linalg.LinAlgError:
                return None
            weighted = scipy.linalg.cho_solve(
                factor, right_hand_side, check_finite=False
            )
            log_determinant = 2.0 * np.log(np.diag(factor[0])).sum()
            return (
                weighted[:, 0] if was_vector else weighted,
        ...
        for position, row in enumerate(rows):
            ...
            try:
                ...
                return None
            local = slice(position * dimension, (position + 1) * dimension)
            blocks.append(scipy.linalg.cho_solve(
                factor, right_hand_side[local], check_finite=False
            ))
            inverse_identity.append(scipy.linalg.cho_solve(
                factor, np.eye(dimension), check_finite=False
            ))
            log_determinant += 2.0 * np.log(np.diag(factor[0])).sum()
        weighted = np.vstack(blocks)
        if cluster_variance > 0:
            summed_inverse = np.sum(inverse_identity, axis=0)
            middle = np.eye(dimension) + cluster_variance * summed_inverse
            try:
                middle_factor = scipy.linalg.cho_factor(
                    middle, lower=True, check_finite=False
                )
            except np.linalg.LinAlgError:
                return None
            summed_weighted = np.sum(
                weighted.reshape(len(rows), dimension, -1), axis=0
            )
            correction = scipy.linalg.cho_solve(
                middle_factor,
                cluster_variance * summed_weighted,
                check_finite=False,
            )
            weighted -= np.vstack([
                inverse @ correction for inverse in inverse_identity
            ])
            log_determinant += (
                2.0 * np.log(np.diag(middle_factor[0])).sum()
            )
        return (
            weighted[:, 0] if was_vector else weighted,
            float(log_determinant),
    ...
    def fit_at_variances(cluster_variance, residual_variance, model_design):
        ...
        for rows in cluster_rows:
            block_design = np.kron(model_design[rows], np.eye(dimension))
            block_values = values[rows].ravel()
            solved = cluster_solve(
                rows,
                cluster_variance,
                residual_variance,
                np.column_stack((block_design, block_values)),
            )
            if solved is None:
                return None
            weighted, local_log_determinant = solved
            weighted_design = weighted[:, :-1]
            weighted_values = weighted[:, -1]
            precision += block_design.T @ weighted_design
            score += block_design.T @ weighted_values
            quadratic_constant += block_values @ weighted_values
            log_determinant += local_log_determinant
        if np.linalg.matrix_rank(precision) != parameter_count:
            return None
        coefficient_covariance = scipy.linalg.inv(
            precision, check_finite=False
        )
        coefficients = coefficient_covariance @ score
        residual_quadratic = quadratic_constant - score @ coefficients
        objective = log_determinant + max(float(residual_quadratic), 0.0)
        return {
    ...
    initial = np.linalg.lstsq(null_design, values, rcond=None)[0]
    residual_scale = float(np.mean(np.square(values - null_design @ initial)))
    covariance_scale = float(
        np.mean(np.trace(covariances, axis1=1, axis2=2)) / dimension
    )
    scale = max(residual_scale, covariance_scale, 1e-8)
    ...
    def optimize_variances(model_design, starts=()):
        candidates = [(0.0, 0.0, fit_at_variances(0.0, 0.0, model_design))]
        for start in starts:
            start = np.maximum(np.asarray(start, dtype=float), 0.0)
            candidates.append((
                float(start[0]),
                float(start[1]),
                fit_at_variances(start[0], start[1], model_design),
            ))

        def add_axis(cluster_axis):
            def objective(log_variance):
                variance = float(np.exp(log_variance))
                fitted = fit_at_variances(
                    variance if cluster_axis else 0.0,
                ...
                return np.inf if fitted is None else fitted["objective"]
            ...
            if optimized.success:
                variance = float(np.exp(optimized.x))
                candidates.append((
                    variance if cluster_axis else 0.0,
                    0.0 if cluster_axis else variance,
                    fit_at_variances(
        ...
        def joint_objective(log_variances):
            fitted = fit_at_variances(
                float(np.exp(log_variances[0])),
                float(np.exp(log_variances[1])),
                model_design,
            )
            return np.inf if fitted is None else fitted["objective"]

        positive_starts = [(scale / 2.0, scale / 2.0)]
        positive_starts.extend(starts)
        for start in positive_starts:
            start = np.maximum(np.asarray(start, dtype=float), np.exp(lower))
            ...
            if optimized.success:
                cluster_variance, residual_variance = np.exp(optimized.x)
                candidates.append((
                    float(cluster_variance),
                    float(residual_variance),
                    fit_at_variances(
                        cluster_variance, residual_variance, model_design
                    ),
                ))
        candidates = [value for value in candidates if value[2] is not None]
        if not candidates:
            raise np.linalg.LinAlgError("Gaussian mixed-model fit failed")
        cluster_variance, residual_variance, fitted = min(
            candidates, key=lambda value: value[2]["objective"]
        )
        fitted["cluster_variance"] = cluster_variance
        fitted["residual_variance"] = residual_variance
        return fitted

    if null_variance_components is None:
        null_fit = optimize_variances(null_design)
    else:
        null_variance_components = np.asarray(
            null_variance_components, dtype=float
        )
        if (
            null_variance_components.shape != (2,)
            or not np.isfinite(null_variance_components).all()
            or np.any(null_variance_components < 0)
        ):
            raise ValueError(
                "null_variance_components must contain two nonnegative values"
            )
        null_fit = fit_at_variances(
            null_variance_components[0],
            null_variance_components[1],
            null_design,
        )
        if null_fit is None:
            raise np.linalg.LinAlgError("fixed Gaussian null fit failed")
        null_fit["cluster_variance"] = float(null_variance_components[0])
        null_fit["residual_variance"] = float(null_variance_components[1])
    alternative_fit = optimize_variances(
        design,
        starts=((
            null_fit["cluster_variance"],
            null_fit["residual_variance"],
        ),),
    )
    statistic = max(
        float(null_fit["objective"] - alternative_fit["objective"]), 0.0
    )
    null_precision = null_fit["precision"]
    null_score = null_fit["score"]
    cross_precision = np.zeros(
        (null_design.shape[1] * dimension, degrees_of_freedom), dtype=float
    )
    tested_precision = np.zeros(
        (degrees_of_freedom, degrees_of_freedom), dtype=float
    )
    tested_score = np.zeros(degrees_of_freedom, dtype=float)
    cluster_variance = null_fit["cluster_variance"]
    residual_variance = null_fit["residual_variance"]
    for rows in cluster_rows:
        nuisance_block = np.kron(null_design[rows], np.eye(dimension))
        tested_block = np.kron(tested_design[rows], np.eye(dimension))
        block_values = values[rows].ravel()
        solved = cluster_solve(
            rows,
            cluster_variance,
            residual_variance,
            np.column_stack((tested_block, block_values)),
        )
        if solved is None:
            raise np.linalg.LinAlgError("fitted null covariance is singular")
        weighted, _ = solved
        weighted_tested = weighted[:, :-1]
        weighted_values = weighted[:, -1]
        cross_precision += nuisance_block.T @ weighted_tested
        tested_precision += tested_block.T @ weighted_tested
        tested_score += tested_block.T @ weighted_values
    nuisance_covariance = scipy.linalg.inv(
        null_precision, check_finite=False
    )
    efficient_score = tested_score - cross_precision.T @ (
        nuisance_covariance @ null_score
    )
    efficient_information = tested_precision - cross_precision.T @ (
        nuisance_covariance @ cross_precision
    )
    efficient_information = 0.5 * (
        efficient_information + efficient_information.T
    )
    if np.linalg.matrix_rank(efficient_information) != degrees_of_freedom:
        raise np.linalg.LinAlgError("tested score information is singular")
    log_bayes_factors = []
    for prior_scale in prior_scales:
        prior_covariance = (
            float(prior_scale ** 2) * prior_covariance_shape
        )
        prior_precision = scipy.linalg.inv(
            prior_covariance, check_finite=False
        )
        posterior_precision = efficient_information + prior_precision
        ...
        if prior_sign <= 0 or posterior_sign <= 0:
            raise np.linalg.LinAlgError("Bayes-factor determinant is invalid")
        log_determinant = prior_log_determinant + posterior_log_determinant
        log_bayes_factors.append(float(
            0.5
            * (
                efficient_score
                @ scipy.linalg.solve(
    ...
    log_bayes_factors = np.asarray(log_bayes_factors)
    mixture_log_bayes_factor = float(
        scipy.special.logsumexp(log_bayes_factors) - np.log(len(prior_scales))
    )
    cluster_count = len(cluster_rows)
    bic_log_bayes_factor = 0.5 * (
        statistic - degrees_of_freedom * np.log(cluster_count)
    )
    return {
        "statistic": statistic,
        "degrees_of_freedom": degrees_of_freedom,
        "p_value": float(scipy.stats.chi2.sf(statistic, degrees_of_freedom)),
        "null_objective": null_fit["objective"],
        "alternative_objective": alternative_fit["objective"],
        "null_cluster_variance": null_fit["cluster_variance"],
        "null_residual_variance": null_fit["residual_variance"],
        "alternative_cluster_variance": alternative_fit["cluster_variance"],
        "alternative_residual_variance": alternative_fit["residual_variance"],
        "coefficients": alternative_fit["coefficients"].reshape(
            design.shape[1], dimension
        ),
        "coefficient_covariance": alternative_fit["coefficient_covariance"],
        "efficient_score": efficient_score,
        "efficient_information": efficient_information,
        "effect_prior_scales": prior_scales,
        "effect_prior_covariance": prior_covariance_shape,
        "log_bayes_factors": log_bayes_factors,
        "mixture_log_bayes_factor": mixture_log_bayes_factor,
        "bic_log_bayes_factor": float(bic_log_bayes_factor),
        "clusters": cluster_count,
    }
