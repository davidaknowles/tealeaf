...
2026-09-26: Restored the deprecated directly matched SUPPA transcript-PSI/Tealeaf split row in the reproducibility table after the native SUPPA2 event audit had replaced it with a comparator-only row. Native SUPPA2 remains separately labeled because its retained split audit has 4,204 genes but no native feature tables for recomputing Tealeaf values on that event universe. Added native SUPPA2 and paired-JCEC rMATS to the long-read method summary and rank audit. Native SUPPA2 gives 1,872/3,871 pooled and 802/2,590 strict directional agreements; paired-JCEC rMATS gives 1,778/3,859 and 660/2,549. Regenerated both method-comparison PDFs and the 200-rank table, then rebuilt the 69-page differential report without missing figures or undefined references.
2026-09-26: Retired the custom transcript-PSI implementation and its reported results after confirming it was not native SUPPA2. The script and artifacts were archived outside the repository; native SUPPA2 is now the only SUPPA comparator in the code, results, and report.
2026-09-26: Reran native SUPPA2 PSI tests on the production subject-fold manifests with the standard 14,752-event catalogue. The fold outputs contain 801,532 and 904,687 event--contrast tests. Simes aggregation within gene--cell-type pair gives 2,342 hypotheses shared with production Tealeaf, 457 shared genes, and the following matched split results: Tealeaf 180/174 fold BH calls, 131 replicated genes, 0.884 held-half nominal replication, and Spearman rho 0.812; native SUPPA2 26/22 fold calls, 1 replicated gene, 0.900 held-half nominal replication, and rho 0.352. The rerun and matched comparison tables are under `analyses/comparator_suppa_rmats/native_suppa2`, with the reusable driver in `extra_scripts/run_suppa2_split_comparison.py`; Table 2 now reports this matched comparison.
2026-09-26: Renamed the report-facing SUPPA2 analyses to full-data SUPPA2 event audit and matched-split SUPPA2 benchmark. The implementation directory retains `native_suppa2` as a provenance path, but “native” is no longer used as the analysis-design label.
