...
Replaced the former high-finite-concentration spike with an exact point null at zero path perturbation and removed the arbitrary finite-slab cap at eight. The candidate grid is now 0.5 through 128. Selection pools all block path dimensions with equal weighting by gene and estimates one finite global smoothing strength outside each of five held-out gene folds. The fitted point-null weight is a nuisance robustifier for selection only; downstream differential testing uses the selected finite strength and does not use spike membership or a posterior probability of differential splicing. Global maps are keyed only by gene fold, so rare block dimensions absent from the evidence subsample still receive the selected strength.

Corrected the Laplace slab evidence across path dimensions. For an (S_b)-path composition represented in an orthonormal ILR basis, the normalized log prior includes (+\tfrac12\log S_b) from the ILR-to-simplex Jacobian. Omitting this term is harmless for alpha selection within a fixed dimension but is incorrect for a point-null mixture pooled across dimensions. After correction, per-path scaling selects four or eight pseudo-observations per path, while fixed-total scaling selects concentrations eight, 16, or 32. Point-null weights range from 0.917 to 0.973.

Reran the complete calibrated pairwise subject-split benchmark under both EB scalings. Per-path EB retains 7,018 and 4,660 eligible tests, makes 97 and 31 fold-specific BH calls, and yields 4/0/0 replicated genes against LeafCutter, MAJIQ Heterogen, and scQuint. Fixed-total EB retains 7,022 and 4,668 tests, makes 98 and 30 BH calls, and also yields 4/0/0 replicated genes. Both have pooled null rejection near 0.05 and zero BH calls in every fold-level null family. Production uniform add-two smoothing gives 1,163 and 770 fold calls and 50/103/101 replicated genes. Concentration scaling therefore does not explain the EB failure. The baseline-centered predictive objective favors pooled composition for the null majority, reduces fit retention by about 500 hypotheses per half, and suppresses discovery-relevant deviations. Corrected EB remains a sensitivity rather than production. Results are under `analyses/path_smoothing_eb`.
