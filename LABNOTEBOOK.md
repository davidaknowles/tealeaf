...
Added `sign_agreement_by_rank.pdf`, a cumulative directional-agreement curve for Tealeaf, LeafCutter, relaxed-coverage MAJIQ Heterogen, scQuint, and paired junction CLR. Each method's eligible calls are independently ordered by its short-read discovery p-value, and the x-axis is the fraction of that method's eligible calls encountered. The curves use the pooled long-read endpoint, requiring complete source representation, at least 20 pooled source UMIs per cell type, and a finite directional indicator. The curve endpoints exactly reproduce the pooled rates in the updated method-comparison table.
