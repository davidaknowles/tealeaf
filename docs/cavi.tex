...
Define the primer exposure of isoform \(t\) by \(\ell_{gpt}=\sum_{k=1}^{K_{gp}}A_{gpkt}\in\mathbb R_+\). Conditional on the primer total, the log likelihood for pseudobulk \(i\) and primer \(p\), including the count-only multinomial constant, is \begin{align}
...
\]
Writing \(\bar x_{gipt}=E_q(x_{gipt})\) and \(v_{git}=\operatorname{Var}_q(\eta_{git})\), the coordinate updates are \begin{align}
 \xi_{gipt}^2
...
For free isoform \(t\leq r_g\), collect the quadratic and linear coefficients \begin{align}
 w_{git}
...
Let \(\mathcal O_{gm}=\{i:m(i)=m\}\). Holding local parameters and model parameters fixed, the coordinate optimum for mouse \(m\)'s full Gaussian factor is \begin{align}
 \Sigma_{gm}^{-1}
 &=P_{gm}^{-1}
...
For point-estimated fixed effects, define \begin{align}
 R_\beta
...
Partition \(\bm\mu_{gm}\) and \(\Sigma_{gm}\) into the random-intercept block \(u\) and active pseudobulk-residual blocks \(e_i\). The empirical-Bayes variance updates are \begin{align}
 \sigma_g^2
...
The package implementation and command-line options were retired after this comparison. This document preserves the derivation, variable dimensions, and negative benchmark result; the supported fitter is the analytic-derivative Laplace objective optimized with bounded L-BFGS.
