...
fixed. Bounded L-BFGS optimizes the exact paired multinomial likelihood,
limiting path perturbations to \(\lvert\delta\rvert\leq20\) to prevent
floating-point underflow at a simplex boundary. Under this parameterization,
\[
...
The minimum gene-assigned UMI count per pseudobulk was varied while retaining
the same covariance and path-proportion reliability rules:
\[
\begin{array}{rrrrrr}
\toprule
...
\end{array}
\]
The 500-UMI permutation estimate is based on only 100 null tests and is
therefore noisy. No cutoff produced a discovery at 5\% FDR.

Asymptotic power was also calculated for reliable two-condition, two-path
tests using their fitted inferential covariance and REML biological variance.
An ILR effect of 0.5, equivalent to a twofold path-odds change, gave mean
conditional-test power of 0.49, 0.51, 0.44, 0.56, and 0.59 at increasing
cutoffs. The apparently higher power at 200 and 500 UMIs is selected from
only eight and two eligible tests. Among 103 test identities shared by the
25- and 50-UMI analyses, power was 0.453 and 0.449, respectively. Thus lower
cutoffs do not degrade calibrated per-test precision after reliability
filtering; they mainly increase the number of testable blocks. A 25-UMI
coverage cutoff is the preferred default for this dataset.
