...
\begin{column}{0.49\textwidth}
\centering
\metric{tlTeal}{894}{nominal omnibus blocks}
\hspace{0.15cm}
\metric{tlOrange}{1,243}{nominal condition tests}
\vspace{0.55cm}
\metric{tlNavy}{791}{omnibus BH blocks}
\hspace{0.15cm}
...
\vspace{0.05cm}
\begin{itemize}\small
\item 1,781 eligible omnibus blocks and 14,833 condition-within-cell-type tests
\item Omnibus calls span 791 blocks and 572 genes
\item Testing-level EB independently selects fixed-total $A=64$ for both families
...
\midrule
LeafCutter & \textbf{73} & \textbf{0.878} & \textbf{0.790} & 26 & 0.750 & 0.542\\
MAJIQ Heterogen & \textbf{156} & \textbf{0.888} & \textbf{0.791} & 38 & 0.698 & 0.437\\
scQuint & \textbf{160} & \textbf{0.875} & \textbf{0.813} & 56 & 0.775 & 0.458\\
\bottomrule
\end{tabular}
\vspace{0.4cm}
\begin{block}{What the paired local-path test changes}
\scriptsize Subject-level path ILRs are estimated directly from both primer-specific EC vectors, stabilized with a globally selected fixed-total concentration, variance-moderated across scalar tests, and empirically calibrated with 32 sign-flip families. Testing-level EB selects $A=32$ for this benchmark. ``Rep.'' applies BH to the conjunction p-value on each pairwise shared gene universe; $\rho$ is fold correlation of gene--pair $-\log_{10}p$.
\end{block}
\begin{alertblock}{Interpretation}
\scriptsize Every aggregated replicated-gene null family has zero BH calls. The complete EB selector was rerun inside each null family. Because $A$ was selected on this benchmark, independent-data confirmation remains necessary.
\end{alertblock}
\end{frame}
...
\hspace{0.12cm}
\metric{tlNavy}{0.49 h}{selected omnibus fit}
\vspace{0.45cm}
\metric{tlOrange}{0.500 GiB}{peak full worker memory}
\hspace{0.12cm}
\metric{tlGold}{5.02 h}{eight-value EB grid}
\end{column}
\begin{column}{0.42\textwidth}
\begin{itemize}\small
...
\end{column}
\end{columns}
\source{Worker time starts from prepared inputs and excludes shared read processing. Full-data values are for the primary omnibus family; the eight EB candidates run independently.}
\end{frame}

\begin{frame}{Production cell-type blocks include 139 cassette exons}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.52\textwidth}
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[x=.032cm,y=.63cm]
\foreach \i/\name/\v/\pct/\clr in {0/Alternative first region/462/58.4/tlTeal,1/Cassette exon/139/17.6/tlOrange,2/Alternative last region/94/11.9/tlNavy,3/Whole-gene complex/41/5.2/tlGold,4/Other/55/7.0/tlGray}{\node[anchor=east,font=\scriptsize] at (-2,4.6-\i*.85) {\name};\fill[\clr!80] (0,4.35-\i*.85) rectangle (\v,4.85-\i*.85);\node[anchor=west,font=\scriptsize] at (\v+2,4.6-\i*.85) {\v\; (\pct\%)};}
\end{tikzpicture}
}
\end{column}
