...
\begin{itemize}[leftmargin=*]
  ...
  The implementation stores these atoms as low-rank factors and uses streamed
  block power iterations on \((B-G\Phi_k)_+\). It performs the quadratic line
  search from compact Gram products, without assembling \(B\) or \(\Phi_k\).
...
\texttt{frank\_wolfe}, and \texttt{factorized}. The scalable methods use a
...
the default Frank--Wolfe radius is \(\sqrt{M}\).
...
is written as thresholded sparse transcript-by-cell chunks, accompanied by the
barcode and transcript order, low-rank factors when applicable, and a manifest
of solver diagnostics. This output layout avoids a dense genome-wide abundance
file and makes reconstruction of selected cell blocks possible from the saved
factors. The downstream intron clustering workflow remains a pseudobulk
operation and is not invoked in this mode.
...
the reference, and the region factor is set to one. A probability-aware EC
reader can later populate \(a_{kt}=p_{kt}\), preserving Salmon's conditional
assignment weights without changing the sparse solver interface.
