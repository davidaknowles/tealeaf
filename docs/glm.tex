...
evidence. It is not a full correction for oligo(dT) positional coverage bias:
the current probability payload does not fit separate primer-specific fragment-
start distributions. A full correction requires primer-separated Salmon
\texttt{--posBias} models, or exporting RAD positions and evaluating a learned
primer-specific positional model when constructing \(A_p\).
