...
For single-cell data, the same gene can be quantified across \(M\) cells at
once. Let
\[
...
\[
  ...
  G = A'A.
\]
Both can be computed with sparse matrix multiplication. The gradient of the
smooth loss is
\[
  \nabla_{\Phi}
  \frac{1}{2}\left\|C-A\Phi\right\|_F^2
  ...
  G\Phi - B,
\]
which requires multiplication by the small isoform-by-isoform matrix \(G\),
rather than by a large dense region-by-cell residual.
...
\begin{itemize}[leftmargin=*]
  ...
  large. In that regime, truncated or randomized SVD, cell minibatches, or the
  factorized formulation below are usually preferable.

  \item \textbf{Frank-Wolfe for a constrained nuclear norm.} For the constrained
  problem
  \[
    \min_{\Phi \ge 0,\,\|\Phi\|_* \le \tau}
    f(\Phi)
    =
    \frac{1}{2}\left\|C-A\Phi\right\|_F^2,
  \]
  Frank-Wolfe avoids nuclear-norm proximal steps. At iteration \(k\), compute
  the gradient
  \[
    D_k = \nabla f(\Phi_k) = G\Phi_k - B.
  \]
  The linear minimization oracle solves
  \[
    S_k
    =
    \arg\min_{S \ge 0,\,\|S\|_* \le \tau}
    \langle D_k, S\rangle.
  \]
  Because \(S\) is constrained to be nonnegative, only the positive part of the
  descent matrix matters. Let
  \[
    H_k = (-D_k)_+ = (B-G\Phi_k)_+.
  \]
  If \(H_k\) is nonzero, take its leading left and right singular vectors
  \(u_k\) and \(v_k\), which can be chosen nonnegative, and set
  \[
    S_k = \tau u_k v_k'.
  \]
  The update
  \[
    \Phi_{k+1}
    =
    (1-\gamma_k)\Phi_k + \gamma_k S_k
  \]
  preserves both nonnegativity and the nuclear-norm constraint. Line search can
  use the same sparse sufficient statistics \(B=A'C\) and \(G=A'A\), since
  \(f(\Phi)\) and directional derivatives are available without forming
  \(C-A\Phi\).

  This approach is attractive when a small effective rank is expected: the
  iterate is an explicit convex combination of rank-one atoms, and each step
  requires only a leading singular vector computation rather than a full SVD.
  The main cost is repeatedly computing the top singular vectors of
  \((B-G\Phi_k)_+\). Randomized or Lanczos methods can exploit fast products
  with \(G\) and the low-rank representation of \(\Phi_k\).
  ...
  \]
  Nonnegativity can be enforced by projecting \(UV'\) onto the nonnegative
  orthant, by using an ADMM split for the constrained matrix, or by
  parameterizing \(\Phi\) through a nonnegative link such as an exponential or
  softplus transformation. Column normalization can be applied after fitting if
  isoform proportions are required.
\end{itemize}

The factorized and Frank-Wolfe approaches are usually the most scalable choices
for many cells. Both keep \(\Phi\) in a compact low-rank representation and use
sparse preprocessing for \(B=A'C\). ADMM is useful as a direct convex reference
implementation for smaller genes or smaller cell batches, but its repeated
singular-value thresholding can be more expensive at large scale.

\subsection*{Choosing the Regularization Level}

The regularization parameter can be chosen by cross-validation because the
many-cell model is a large regression problem. A natural split is over
regions, or equivalently over read equivalence classes. For a fold \(q\), split
the rows of \(A\) and \(C\) into training and validation sets:
\[
  A =
  \begin{bmatrix}
    A_{\mathrm{tr}} \\
  ...
  \end{bmatrix}.
\]
For each candidate penalty \(\lambda\), or each candidate nuclear-norm radius
\(\tau\), fit \(\Phi\) using only the training rows. For the constrained form,
this means solving
\[
  \hat{\Phi}_{\tau}^{(q)}
  =
  \arg\min_{\Phi \ge 0,\,\|\Phi\|_* \le \tau}
  \frac{1}{2}\left\|C_{\mathrm{tr}} - A_{\mathrm{tr}}\Phi\right\|_F^2.
\]
Then evaluate prediction error on the held-out rows:
\[
  \operatorname{CV}_q(\tau)
  =
  \left\|
    C_{\mathrm{val}}
    -
    A_{\mathrm{val}}\hat{\Phi}_{\tau}^{(q)}
  \right\|_F^2.
\]
The selected regularization level minimizes the average validation error across
folds. A weighted version can be used if some regions or cells have much higher
read depth:
\[
  \operatorname{CV}_q(\tau)
  =
  ...
  \right\|_F^2,
\]
where \(W_{\mathrm{val}}\) contains inverse-variance or read-depth weights and
\(\odot\) denotes elementwise multiplication.

Row-wise folds test whether isoform usage inferred from one set of
equivalence classes predicts held-out equivalence classes for the same cells.
The folds should be stratified so that each training set retains informative
regions for the gene; otherwise a fold can remove all evidence distinguishing
two isoforms. Holding out individual sparse entries is possible, but holding
out rows is usually cleaner because a row corresponds to a biological read
compatibility pattern and avoids mixing the same equivalence class between
training and validation.

\section*{Remarks}

\begin{itemize}[leftmargin=*]
  \item The derivation intentionally starts from a simplified model that only
  uses isoform of origin and read start site. Full read-based methods can add
  information such as fragment length, orientation, or GC content.
  \item In the basic model, equivalence classes coincide with disjoint regions
  because all reads in a region have the same mapping set and likelihood
  contribution.
  \item More complex methods may define equivalence classes differently, but
  read counts over equivalence classes can still serve as sufficient statistics
  for isoform abundance estimation.
\end{itemize}
