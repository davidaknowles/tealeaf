...
For molecular-abundance regularization the two primer designs retain separate
effective-length transformations,
\[
  A_{\theta,p}=A_{\phi,p}
  \operatorname{diag}(\tilde{\bm l}_p).
\]
This is necessary because positional bias changes the sampled effective
length differently under oligo(dT) and random-hexamer priming. Using one
reference-derived length vector for both primers would partially discard the
correction. The fitted \(\Theta\) remains shared across the paired halves.
