...
\begin{itemize}[leftmargin=*]
  ...
  \]
  The implementation uses nonnegative factors and projected cell-minibatch
  updates. In a stochastic epoch it visits every cached cell block once in a
  seeded random order. Each block first updates its rows of \(V\), then gives
  a cell-averaged incremental gradient update for the shared factor \(U\).
  Curvature-bounded projected steps are used for both updates. Before a fit is
  declared converged, it switches to deterministic full-gradient alternating
  epochs and applies the patient objective stopping rule there. Paired factor
